...
func TestGrammarCompositionOrder(t *testing.T) {
	table := []struct {
		...
		{g: Grammar{Path: "", Rules: map[string]Rule{}, Imports: []string{}}, exp: []string{}},
		{g: Grammar{Path: "", Rules: map[string]Rule{"<a>": NewRule("", false)}, Imports: []string{}}, exp: []string{}},
		{
		...
		{
			g: Grammar{
				Path: "", Rules: map[string]Rule{"<a>": NewRule("", true), "<b>": NewRule("", true)},
				Imports: []string{},
			}, exp: []string{"<a>", "<b>"},
		},
		{
			g: Grammar{
				Path: "", Rules: map[string]Rule{"<a>": NewRule("", true), "<b>": NewRule("", false)},
				Imports: []string{},
			}, exp: []string{"<a>"},
		},
		{
			g: Grammar{
				Path: "", Rules: map[string]Rule{"<a>": NewRule("<b>", true), "<c>": NewRule("", true)},
				Imports: []string{},
			}, exp: []string{"<a>", "<b>", "<c>"},
		},
		{
			g: Grammar{
				Path: "", Rules: map[string]Rule{"<a>": NewRule("<b>", true), "<c>": NewRule("", false)},
				Imports: []string{},
			}, exp: []string{"<a>", "<b>"},
		},
		{
		...
		g.Rules = map[string]Rule{
			...
			"<e>": {
				Exp: "1(2[3]);", IsPublic: false, References: []string{}, Graph: NewGraph(EdgeList{
					...
					{From: 5, To: 6, Weight: 1.0}, {From: 6, To: 7, Weight: 1.0}, {From: 7, To: 8, Weight: 1.0},
			...
			"<i>": {
				Exp: "a<b>c;", IsPublic: false, References: []string{"<b>"}, Graph: NewGraph(EdgeList{
					{From: 0, To: 1, Weight: 1.0}, {From: 1, To: 2, Weight: 1.0}, {From: 2, To: 3, Weight: 1.0},
					{From: 3, To: 4, Weight: 1.0}, {From: 4, To: 5, Weight: 1.0},
